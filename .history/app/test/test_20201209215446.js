var assert = require('assert');
var calificar = require('../models/puntos')

describe('Puntuacion', function() {

  describe('criterioAbandono()', function() {

    it('debe retornar calificar', function() {
        assert.equal(typeof oPuntuacion, object);
        assert.equal(typeof oPuntuacion.calificar, 'function');
      });

    it('debe retornar -100 cuando mensajes.length es igual a 1', function() {
      assert.equal(mensajes.length, 1);
    });

    it('debe retornar la primera ocurrencia del valor especificado', function() {
      assert.equal(cantMinutos < 1,  50);
    });

  });
});